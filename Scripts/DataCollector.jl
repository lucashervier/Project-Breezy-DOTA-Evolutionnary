function InitializeDF()
	...
	# we add the action column
	df[!, :action] .= Int64[]
	return df
end

function AddRow(df::DataFrame,lastFeature::Array{Any},action::Int64)
	push!(df,[lastFeature;action])
end

function SaveCSV(df::DataFrame;name::String="None")
	if (name=="None")
		fileName = Dates.now()
		fileName = Dates.format(fileName, "dd-mm-yyyy-HH-MM-SS")
	else
		fileName = name
	end
